...
$fontSize = 32
$VTFSize = "128x32"
$imagesFolder = "$PSScriptRoot\numberscolored"  # Assuming the images are stored in a folder named "numbers" in the script's directory
...
for ($num = 0; $num -le $maxNumber; $num++) {
    ...
    # Generate image
    magick.exe -auto-orient -background $backgroundColor -fill "$textColor" -font $font -size $VTFSize -gravity center -pointsize $fontSize -antialias label:$num "$imagesFolder\$num.png"
    # Update progress bar
...
for ($num = 1105; $num -le 1115; $num++) {
    ...
    ./im_convert.exe -auto-orient -background $backgroundColor -fill "$textColor" -font $font -size $VTFSize -gravity center -pointsize $fontSize -antialias label:$num "$imagesFolder\$num.png"
}
